 ...
 * =====================================================================================
 *
 *       Filename:  011-lerosua.c
 *
 *    Description:  
 *
 *        Version:  1.0
 *        Created:  2009年10月22日 09时37分37秒
 *       Revision:  none
 *       Compiler:  gcc
 *
...
{24, 47, 32, 60, 99, 3, 45, 2, 44, 75,
 33, 53, 78, 36, 84, 20, 35, 17, 12, 50}, {32, 98, 81, 28, 64,
					   23, 67, 10, 26, 38, 40, 67,
...
{20,
 73, 35, 29, 78, 31, 90, 1, 74, 31, 49, 71, 48, 86, 81, 16, 23,
 57, 5, 54}, {1, 70, 54, 71, 83, 51, 54, 69, 16, 92, 33, 48, 61,
	      43, 52, 1, 89, 19, 67, 48}
};
...
#endif
			if(j<17){
				int hen=num[i][j]*num[i][j+1]*num[i][j+2]*num[i][j+3];
				if(hen>max)
					max=hen;
			}
			if(i<17){
				int su= num[i][j]*num[i+1][j]*num[i+2][j]*num[i+3][j];
				if(su>max)
					max=su;
			}
			if(i<17 && j<17){
				int zuo=num[i][j]*num[i+1][j+1]*num[i+2][j+2]*num[i+3][j+3];
				if(zuo>max)
					max=zuo;
			}
			if(i>3&&j<17){
				int you=num[i][j]*num[i-1][j+1]*num[i-2][j+2]*num[i-3][j+3];
				if(you > max)
					max=you;
			}
		}
